...
import util.commonUtil.interfaces.IFormatToString;
...
public class ComLogUtil extends CommonUtil{
	...
	private static IConfigManager configManager = ConfigManager.getConfigManager();
	private static boolean overwriteSYSO = configManager.getBoolean("overwriteSYSO");
	
	static {
		if(overwriteSYSO) {
			try {
				info = new PrintStream(new FileOutputStream(ComFileUtil.createFile("c:\\Logs\\DoLogs_" + ComDateUtil.getDateAggreated() + ".txt")
//						; new File(
...
//						)
						 ...
						 */
						, configManager.getBoolean("append2newLog")
...
//    	System.out.println(sb);
	...
	private static String getPackageNameFrmSte(StackTraceElement ste) {
		String fn = ste.getClassName();
		return fn.substring(0, fn.lastIndexOf("."));
	}
	
	private static String getClassNameFrmSte(StackTraceElement ste) {
		String fn = ste.getClassName();
	...
	private static class ObjectToStringer {
		...
		static {
			 ...
			 */
			excludePkgs.addAll(configManager.getList("excludePkgs"));
			/**
			 * itself cannot be one of those Class
			 */
			excludeClasses.addAll(configManager.getList("excludeClasses"));
			
			/**
			 * itself and it's parant(implemented interfaces and superclasses) cannot be one of those Class
			 */
			excludeFatherClasses.addAll(configManager.getList("excludeFatherClasses"));
		}
		...
		private String objToFormatString(Object obj) {
			Object res = this.wrap(obj, this.depth4Collection2Expand, this.depth4NoneCollection2Expand);
			return IFormatToString.class.isAssignableFrom(res.getClass()) ? ((IFormatToString)res).toString(2) : res.toString();
		}
...
//					this.loopLog.append('"').append(this.Key2Str(key)).append('"').append(":").append(objToString(value, depth4Collection2Expand, depth4NoneCollection2Expand));
	...
	public static String objToString(Object obj, int depth4Collection2Expand, int depth4NoneCollection2Expand, Class<?>[] exclude2ExpandClasses, Class<?>[] hashcodeOnlyClasses) {
		return getObjectToStringer(depth4Collection2Expand, depth4NoneCollection2Expand, exclude2ExpandClasses, hashcodeOnlyClasses).objToString(obj);
	}
	
	
	public static String objToFormatString(Object obj) {
		return objToFormatString(obj, 1, 1, null, null);
	}
	
	public static String objToFormatString(Object obj, int depth4Collection2Expand, int depth4NoneCollection2Expand) {
		return objToFormatString(obj, depth4Collection2Expand, depth4NoneCollection2Expand, null, null);
	}
	
	public static String objToFormatString(Object obj, int depth4Collection2Expand, int depth4NoneCollection2Expand, Class<?>[] exclude2ExpandClasses) {
		return objToFormatString(obj, depth4Collection2Expand, depth4NoneCollection2Expand, exclude2ExpandClasses, null);
	}
	
	public static String objToFormatString(Object obj, int depth4Collection2Expand, int depth4NoneCollection2Expand, Class<?>[] exclude2ExpandClasses, Class<?>[] hashcodeOnlyClasses) {
		return getObjectToStringer(depth4Collection2Expand, depth4NoneCollection2Expand, exclude2ExpandClasses, hashcodeOnlyClasses).objToFormatString(obj);
	}
	
	private static ObjectToStringer getObjectToStringer(int depth4Collection2Expand, int depth4NoneCollection2Expand, Class<?>[] exclude2ExpandClasses, Class<?>[] hashcodeOnlyClasses) {
		return new ObjectToStringer(depth4Collection2Expand, depth4NoneCollection2Expand
				, exclude2ExpandClasses == null ? null : new ArrayList<Class<?>>(Arrays.asList(exclude2ExpandClasses))
				, hashcodeOnlyClasses == null ? null : new ArrayList<Class<?>>(Arrays.asList(hashcodeOnlyClasses)));
	}
